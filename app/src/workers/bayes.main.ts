...
import { WorkerJob } from '../jobqueue/queue/bayes.queue';
import { Job, QueueScheduler, Worker } from 'bullmq';
import { JobStatus, BayesJobsModel } from '../jobs/models/bayes.jobs.model';
import * as path from 'path';
import { BayesModel } from '../jobs/models/bayes.model';
...
export const createWorkers = async (
  ...
  for (let i = 0; i < config.numWorkers; i++) {
    ...
    worker.on('completed', async (job: Job, returnvalue: any) => {
      ...
      const resultsFile = `${pathToOutputDir}/file_results.txt`;

      //update db with result files
      const finishedJob = await BayesJobsModel.findByIdAndUpdate(
        job.data.jobId,
        {
          status: JobStatus.COMPLETED,
          bayesResultsFile: resultsFile,
